...
def main(args):
    ...
    with mlflow.start_run():
        ...
        # === BASIC DATA VALIDATION (replacing Great Expectations) ===
        ...
        if missing_count > 0:
            is_valid = False
            failed["missing_values"] = int(missing_count)

        # Check 2: Target column exists
        if "Churn" not in df.columns:
            is_valid = False
            failed["missing_target"] = "Churn column not found"

        # Log validation result to MLflow
        mlflow.log_metric("data_quality_pass", int(is_valid))

        if not is_valid:
            import json
            mlflow.log_text(json.dumps(failed, indent=2), artifact_file="data_validation_errors.json")
